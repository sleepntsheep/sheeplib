 ...
 * @cmpfnc three-way comparison function, returning negative if a < b, 0 if a ==
 * b, positive if a > b
 */
#define SALGO_DECL(type, cmpfnc)                                               \
    ...
    /**                                                                        \
     * @brief binary search and return pointer to element if exists, NULL      \
     * otherwise                                                               \
     */                                                                        \
    type *sbsearch_##type(type key, type *base, size_t nmemb);                 \
    /**                                                                        \
     * @brief binary search for first element greater than key and return      \
     * pointer to element if exists, NULL otherwise                            \
     */                                                                        \
    type *supperbound_##type(type key, type *base, size_t nmemb);              \
    /**                                                                        \
     * @brief binary search for first element not greater than key and return  \
     * pointer to element if exists, NULL otherwise                            \
     */                                                                        \
    type *slowerbound_##type(type key, type *base, size_t nmemb);              \
    /**                                                                        \
     * @brief linear search and return pointer to element if exists, NULL      \
     * otherwise                                                               \
     */                                                                        \
    type *slsearch_##type(type key, type *base, size_t nmemb);                 \
                                                                               \
    void ssort_merge_##type(type *base, size_t left, size_t mid,               \
                            size_t right) {                                    \
